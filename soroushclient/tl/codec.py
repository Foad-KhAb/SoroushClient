...
def _write_value(w: TLWriter, type_: str, val: Any, skip_cid: bool = False):
    ...
    if type_ == "int":       w.write_int(val)
    elif type_ == "long":    w.write_long(val)
    elif type_ == "double":  w.write_double(val)
    elif type_ == "int128":  w.write_raw(val if isinstance(val, bytes) else val.to_bytes(16, "little"))
    elif type_ == "int256":  w.write_raw(val if isinstance(val, bytes) else val.to_bytes(32, "little"))
    elif type_ == "string":  w.write_string(val)
    elif type_ == "bytes":   w.write_bytes(val)
...
def _deserialize(cls: Type[TLObject], r: TLReader) -> TLObject:
    ...
    for f in cls.FIELDS:
        ...
        try:
            if f.is_vector:
                r.read_int(signed=False)  # vector constructor
                count = r.read_int()
                kwargs[f.name] = [_read_value(r, f.type, f.skip_cid) for _ in range(count)]
            else:
                kwargs[f.name] = _read_value(r, f.type, f.skip_cid)
        except Exception as e:
            logger.info(f"[tl] Error deserializing {cls.__name__}.{f.name} (type={f.type}) at pos={r._pos}: {e}")
            kwargs[f.name] = None
            break
    for f in cls.FIELDS:
...
def _read_value(r: TLReader, type_: str, skip_cid: bool = False) -> Any:
    if type_ == "int":    return r.read_int()
    elif type_ == "long": return r.read_long()
    elif type_ == "double": return r.read_double()
    elif type_ == "int128": return r.read_raw(16)
    elif type_ == "int256": return r.read_raw(32)
    elif type_ == "string": return r.read_string()
    elif type_ == "bytes":  return r.read_bytes()
    elif type_ == "bool":
        cid = r.read_int(signed=False)
        return cid == 0x997275b5
    elif type_ == "true": return True
    else: return TLObject.read_object(r)
